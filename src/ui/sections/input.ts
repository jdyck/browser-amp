...
function connectionLabel(snapshot: AudioSnapshot): string {
  ...
  if (snapshot.lifecycle === 'monitoring') {
    return 'Connected — monitoring';
  }
  if (snapshot.lifecycle === 'connected-muted') {
...
function connectionDescription(snapshot: AudioSnapshot): string {
  if (snapshot.lifecycle === 'monitoring') {
    return 'Input is connected and Processed Monitoring is on.';
  }

  if (snapshot.lifecycle === 'connected-muted') {
    return 'Input is connected and metering. Processed Monitoring is off.';
  }

  if (snapshot.lifecycle === 'interrupted') {
    return 'Audio was interrupted. Resume Processed Monitoring when you are ready.';
  }

  if (snapshot.lifecycle === 'connecting') {
...
function deviceSelector(snapshot: AudioSnapshot): string {
  const selectedUnavailable = unavailableDeviceOption(
    ...
    .map(
      (device) => `
        <option
          ...
          ${device.id === snapshot.selectedInputDeviceId ? 'selected' : ''}
        >
  ...
  const firstLabel =
    snapshot.devices.length === 0
      ? 'Connect to discover inputs'
      : 'System default';

  return `
    <label class="${FIELD}" for="input-device">
      Input device
    </label>
    ...
    <!--
    <span id="device-help" class="${FIELD_HELP}">
      Choose the interface or microphone feeding the amp.
    </span>
    -->
  `;
}
...
function channelSelector(snapshot: AudioSnapshot): string {
  const options = Array.from(
    { length: snapshot.inputChannelCount },
    (_, channel) => `
      <option
        value="${channel}"
        ${channel === snapshot.inputChannel ? 'selected' : ''}
      >
        Channel ${channel + 1}
      </option>
    `,
  ...
  return `
    <label class="${FIELD}" for="input-channel">
      Input Channel
    </label>
  ...
  action(snapshot, recovery) {
    return `
      <div class="section-action">
        <output class="connection-state" role="status">
        ...
        <button
          ...
          ${snapshot.lifecycle === 'connecting' ? 'disabled' : ''}
        >
          ${recovery.connectButtonLabel}
        </button>
  ...
  content(snapshot, recovery) {
    return `
      <div class="section-stack">
        <section class="panel" aria-labelledby="input-title">
          ...
          <div class="panel-heading">
            <div>
              <p class="panel-eyebrow">Source</p>
              <h2 id="input-title">Live Guitar Input</h2>
            </div>
            <span class="placeholder-art" aria-hidden="true">
              [img]
            </span>
          </div>

          <p id="connection-description" class="panel-description">
            ${connectionDescription(snapshot)}
          </p>
          -->

          <div class="select-grid">
            <div>
              ${deviceSelector(snapshot)}
            </div>

            ${
      snapshot.inputChannelCount > 1
        ? `
                  <div>
                    ${channelSelector(snapshot)}
                  </div>
                `
        : ''
    }
      ...
      .join('')}
          ...
          ${
      recovery.inputMessage === undefined
        ? ''
        : `
                <p class="message error" role="alert">
                  ${escapeHtml(recovery.inputMessage)}
                </p>
              `
    }
        </section>
      ...
      'Set the level feeding the amp without clipping the input.',
    )}
        </section>
      ...
      'Set the maximum reduction during quiet passages.',
    )}
      ...
      'ms',
      'Control how gradually the gate settles.',
    )}
          </div>
          ...
          <div class="live-readout">
            ...
            <strong
              id="noise-gate-reduction"
              aria-label="Noise suppression reduction"
            >
              ${snapshot.noiseGateReductionDb.toFixed(1)} dB
            </strong>
          </div>
        </section>
      </div>
  ...
  bind(runtime) {
    ...
    runtime.root
      ...
      ?.addEventListener('click', () => {
        void runtime.engine.connectInput({
          deviceId: current().selectedInputDeviceId,
        });
      });
    ...
    runtime.root
      .querySelector<HTMLSelectElement>('#input-device')
    ...
    runtime.root
      .querySelector<HTMLSelectElement>('#input-channel')
      ?.addEventListener('change', (event) => {
    ...
    runtime.root
      .querySelector<HTMLInputElement>('#noise-gate-enabled')
      ?.addEventListener('change', (event) => {
        runtime.engine.applyControls({
          ...current().controls,
          noiseGateBypassed: !(
