...
export function createReverbSection(): WorkspaceSectionModule {
  ...
  function accordions(
    controls: SectionRuntime['engine']['snapshot']['controls'],
  ): string {
    const parameters = reverbParameters(
      controls.reverbProfile,
      controls.reverbSettings,
    );

    return (['main', 'advanced'] as const)
      .map((section) => {
        const amountControl =
          section === 'main'
            ? percentControl(
                ...
                AMP_CONTROL_DEFINITIONS.reverbAmount,
                'Set how much ambience is mixed in.',
              )
            : '';

        const profileControls = reverbControlEntries(
          controls.reverbProfile,
          section,
        )
          .map(([key, definition]) =>
        ...
        return `
          <details
            id="reverb-${section}"
            class="reverb-accordion"
            ${accordionOpen[section] ? 'open' : ''}
          >
            <summary>
              ${section === 'main' ? 'Main Controls' : 'Advanced Controls'}
            </summary>
  ...
  function parameterControl(
    id: string,
    value: number,
    definition: ReverbControlDefinition,
  ): string {
  ...
  function bindControls(runtime: SectionRuntime): void {
    ...
    for (const section of ['main', 'advanced'] as const) {
      const details = runtime.root.querySelector<HTMLDetailsElement>(
        `#reverb-${section}`,
      );
    ...
    for (const [key] of reverbControlEntries(
      current().controls.reverbProfile,
    )) {
      bindContinuousControl(
        runtime.root,
        `reverb-${key}`,
  ...
  const module: WorkspaceSectionModule = {
    ...
    action(snapshot) {
      return stageToggle(
        'reverb-enabled',
        'Enable Reverb',
        !snapshot.controls.reverbBypassed,
      );
    },
    ...
    content(snapshot) {
      ...
      return `
        <div class="section-stack">
          <section
            class="panel reverb-choice-panel"
            aria-label="Reverb"
          >
            <div class="panel-heading compact">
              ...
              <button
                ...
                type="button"
                class="text-action"
                title="Restore only this module's Main and Advanced settings. Amount and Enable Reverb stay unchanged."
              >
                Reset This Reverb
              </button>
            </div>
            ...
            <span
              id="reverb-profile-help"
              class="choice-help"
            >
              ${REVERB_PROFILES[controls.reverbProfile].description}
            </span>
          </section>

          <section
            id="reverb-settings"
            class="panel control-panel reverb-control-panel"
            data-profile="${controls.reverbProfile}"
            aria-label="Reverb Controls"
          >
            ${accordions(controls)}
          </section>
        </div>
    ...
    bind(runtime) {
      ...
      runtime.root
        .querySelector<HTMLSelectElement>('#reverb-profile')
      ...
      runtime.root
        .querySelector<HTMLButtonElement>('#reset-reverb')
        ?.addEventListener('click', () => {
          ...
          runtime.engine.applyControls({
            ...
            reverbSettings: {
              ...controls.reverbSettings,
              [controls.reverbProfile]: {
                ...DEFAULT_REVERB_SETTINGS[controls.reverbProfile],
              },
            },
          });
        });
      ...
      runtime.root
        .querySelector<HTMLInputElement>('#reverb-enabled')
        ?.addEventListener('change', (event) => {
          runtime.engine.applyControls({
            ...current().controls,
            reverbBypassed: !(
    ...
    sync(runtime, snapshot) {
      ...
      for (const [key, definition] of reverbControlEntries(
        controls.reverbProfile,
      )) {
        setControlValue(
          root,
