...
export function choiceSelector(
  ...
  const choiceCards = Object.entries(options)
    .map(
      ([value, option]) => `
        <button
          type="button"
          class="choice-card ${value === selected ? 'is-selected' : ''}"
          data-select-id="${id}"
  ...
  root
    ...
    .forEach((button) => {
      button.addEventListener('click', () => {
        ...
        if (
          select === null ||
          button.dataset.selectValue === undefined
        ) {
          return;
        }
...
export function bindContinuousControl(
  ...
  numeric?.addEventListener('input', () => {
    if (Number.isFinite(numeric.valueAsNumber)) {
      apply(numeric.valueAsNumber);
    }
  });
...
export function syncChoiceCards(
  root: HTMLElement,
  id: string,
  value: string,
): void {
...
export function setControlValue(
  ...
  if (slider !== null && slider.valueAsNumber !== value) {
    slider.value = String(value);
  }

  const formatted = value.toFixed(definition.fractionDigits);

  if (numeric !== null && numeric.value !== formatted) {
    numeric.value = formatted;
  }
}

export function dbControl(
  id: string,
  label: string,
  value: number,
  definition: ContinuousControlDefinition,
...
export function knobControl(
  ...
  label: string,
  value: number,
  definition: AmpKnobDefinition,
): string {
  return continuousControl(id, label, value, definition, '');
}
    ...
    <div class="continuous-control">
      ...
      <div class="value-field">
        <input
          ...
          max="${definition.maximum}"
          step="${definition.step}"
          value="${value.toFixed(definition.fractionDigits)}"
        >
