...
// Cache for computed saturation curves
const curves = new Map<string, Float32Array<ArrayBuffer>>();

/**
 * Computes a saturation curve for nonlinear wave shaping.
...
function topologyKey(model: AmpModel, settings: JazzAmpSettings): string {
  switch (model) {
    case 'amp.studio-clean-v1':
      return `${model}/${settings[model].headroom}`;
    case 'amp.warm-jazz-combo-v1':
      return `${model}/${settings[model].color}/${settings[model].input}`;
    case 'amp.blackface-combo-v1':
      return `${model}/${settings[model].bright}`;
    case 'amp.high-headroom-american-v1':
      return `${model}/${settings[model].bright}/${settings[model].headroom}`;
    case 'amp.small-tweed-combo-v1':
      return `${model}/${settings[model].input}`;
    case 'amp.british-chime-v1':
      return `${model}/${settings[model].channel}`;
  }
}
...
class AmpPath implements StagePath {
  ...
  constructor(context: BaseAudioContext, model: AmpModel, settings: JazzAmpSettings[AmpModel]) {
    ...
    // Build the signal path for the selected amp model
    switch (model) {
      case 'amp.studio-clean-v1':
        this.#buildStudio(settings as StudioCleanSettings);
        break;

      case 'amp.warm-jazz-combo-v1':
        this.#buildWarmJazz(settings as WarmJazzComboSettings);
        break;

      case 'amp.blackface-combo-v1':
        this.#buildBlackface(settings as BlackfaceComboSettings);
        break;

      case 'amp.high-headroom-american-v1':
        this.#buildHighHeadroom(settings as HighHeadroomAmericanSettings);
        break;

      case 'amp.small-tweed-combo-v1':
        this.#buildSmallTweed(settings as SmallTweedComboSettings);
        break;

      case 'amp.british-chime-v1':
        this.#buildBritishChime(settings as BritishChimeSettings);
        break;
    }
  }
  ...
  setControls(settings: JazzAmpSettings[AmpModel]): void {
    ...
    switch (this.#model) {
      case 'amp.studio-clean-v1': {
        ...
        set('bass', toneDb(state.bass, 12));
        set('middle', toneDb(state.middle, 12));
        set('treble', toneDb(state.treble, 12));
        break;
      }
      ...
      case 'amp.warm-jazz-combo-v1': {
        ...
        set('bass', toneDb(state.bass, 10));
        set('middle', toneDb(state.middle, 9));
        set('treble', toneDb(state.treble, 10));
        break;
      }
      ...
      case 'amp.blackface-combo-v1': {
        ...
        set('bass', (state.bass - 4) * 1.7);
        set('treble', (state.treble - 5.5) * 1.7);
      ...
      case 'amp.high-headroom-american-v1': {
        ...
        setDb('drive', (state.volume - 4) * 3);
        set('bass', (state.bass - 4) * 1.6);
        set('middle', toneDb(state.middle, 8));
        set('treble', (state.treble - 5.5) * 1.6);
      ...
      case 'amp.small-tweed-combo-v1': {
        ...
        setDb('drive', (state.volume - 3.5) * 4);
        set('tone', toneDb(state.tone, 10));
        break;
      }
  ...
  #buildStudio(state: StudioCleanSettings): void {
    const path: AudioNode[] = [
      ...
      this.#controlledGain(
        'drive',
        dbToLinearGain(studioGainDb(state.gain))
      ),
  ...
  #buildBlackface(state: BlackfaceComboSettings): void {
    this.#connectPath([
      ...
      this.#controlledGain('drive', dbToLinearGain((state.volume - 4) * 3.3)),
      this.#controlledFilter(
        'bright',
        'highshelf',
        2_700,
  ...
  #buildHighHeadroom(state: HighHeadroomAmericanSettings): void {
    ...
    this.#connectPath([
      ...
      this.#controlledGain(
        'drive',
        dbToLinearGain((state.volume - 4) * 3)
      ),
      this.#controlledFilter(
        'bright',
        'highshelf',
  ...
  #buildBritishChime(state: BritishChimeSettings): void {
    ...
    this.#connectPath([
      ...
      this.#controlledFilter(
        'bass',
        'lowshelf',
        120, (state.bass - 4) * 1.7 - (topBoost ? 1 : 0)
      ),
      this.#filter(
        'peaking',
...
function toneDb(knob: number, range: number): number {
