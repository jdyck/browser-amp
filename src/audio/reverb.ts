...
export class ReverbStage {
  ...
  readonly #impulses = new Map<ReverbProfile, {
    key: string;
    buffer: AudioBuffer
  }>();
  #selection: Selection;
  ...
  constructor(
    ...
    parameters = reverbParameters(profile)) {
    ...
    this.#switcher = new StageSwitcher(
      context,
      bypassed ? 'off' : this.#selection,
      (selection) => this.#createPath(selection));
  ...
  setControls(
    ...
    bypassed: boolean,
    profile: ReverbProfile,
    parameters = reverbParameters(profile)): void {
    if (amount !== this.#amount) {
      this.#amount = amount;
      smoothGainToValue(
        this.#wetGain.gain,
        amount / 100 * REVERB_MAX_WET_GAIN,
  ...
  #createPath(selection: 'off' | Selection): StagePath {
    ...
    const {
      dwell: _dwell,
      modulationDepth: _depth,
      modulationRateHz: _rate,
      ...impulseParameters
    } = parameters;
    ...
    if (cached?.key !== impulseKey) {
      cached = {
        key: impulseKey,
        buffer: createReverbImpulse(this.#context, profile, parameters)
      };
