...
} satisfies Record<ReverbProfile, ImpulseFactory>;
...
export function createReverbImpulse(
  ...
  if (profile === 'studio-chamber' && parameters.preDelayMs > 0) {
    impulse = applyPreDelay(context, impulse, parameters.preDelayMs);
  }

  if (parameters.toneDb !== 0) {
    applyTone(impulse, parameters.toneDb, context.sampleRate);
  }
...
function applyPreDelay(
  ...
  const offset = Math.round(
    (preDelayMs / 1000) * context.sampleRate,
  );
  const delayed = context.createBuffer(
    2,
    impulse.length + offset,
    context.sampleRate,
  );
  ...
  for (let channel = 0; channel < 2; channel += 1) {
    delayed
      ...
      .set(impulse.getChannelData(channel), offset);
  }
...
function applyTone(
  ...
  const memory = Math.exp(-2 * Math.PI * 3_200 / sampleRate);
  const gain = 10 ** (toneDb / 20);
...
function createPlateImpulse(
  ...
  const length = Math.round(
    context.sampleRate *
      (parameters.decaySeconds +
        Math.max(0, parameters.preDelayMs / 1000 - 0.012)),
  );
  const impulse = context.createBuffer(2, length, context.sampleRate);
  const preDelaySamples = Math.round(
    (context.sampleRate * parameters.preDelayMs) / 1000,
  );
  ...
  for (let channel = 0; channel < impulse.numberOfChannels; channel += 1) {
    ...
    for (let index = 0; index < samples.length; index += 1) {
      ...
      const progress =
        (index - preDelaySamples) /
        Math.max(1, samples.length - preDelaySamples - 1);
      const damping =
        2 ** ((50 - parameters.damping) / 25) *
        (1 -
          (0.8 * Math.max(0, parameters.damping - 50) * progress) / 50);
      ...
      samples[index] =
        ...
        envelope *
        IMPULSE_GAIN *
        Math.sqrt(IMPULSE_DURATION_SECONDS / parameters.decaySeconds);
    }
  }
...
function createSyntheticImpulse(
  ...
  const impulse = context.createBuffer(
    2,
    Math.round(context.sampleRate * duration),
    context.sampleRate,
  );
  ...
  for (let channel = 0; channel < 2; channel += 1) {
    ...
    for (let index = 0; index < samples.length; index += 1) {
      ...
      const fade = Math.min(
        1,
        (samples.length - 1 - index) / (context.sampleRate * 0.02),
      );
      samples[index] = highpass * fade;
      energy += samples[index] ** 2;
    }

    const gain =
      energy > 0
        ? Math.sqrt(32 * (context.sampleRate / 48_000) / energy)
        : 0;
...
function createRoomImpulse(
  ...
  const reflections = chamber
    ? [0.009, 0.016, 0.027, 0.043, 0.061]
    : [0.005, 0.011, 0.019, 0.029];

  return createSyntheticImpulse(
    context,
    duration,
    (samples, channel, sampleRate) => {
      const noise = randomNoise(
        (chamber ? 0x4348414d : 0x524f4f4d) + channel * 0x1021,
      );
      const densityNoise = randomNoise(0x44494646 + channel);
      const density = Math.min(
        1,
        0.02 + 0.98 * (parameters.diffusion / 70) ** 2,
      );
      const onset = (chamber ? 0.022 : 0.018) * size;
      let filtered = 0;

      for (
        let index = Math.round(onset * sampleRate);
        index < samples.length;
        index += 1
      ) {
        const time = index / sampleRate - onset;
        const memory = Math.exp(
          (-2 * Math.PI * (chamber ? 2_800 : 1_800)) /
            (sampleRate * (1 + 4 * time)),
        );
        const value = noise();
        const diffuseValue =
          densityNoise() < 2 * density - 1
            ? value / Math.sqrt(density)
            : 0;

        filtered = memory * filtered + (1 - memory) * diffuseValue;

        const attack = Math.min(
          1,
          time / (0.025 * 70 / Math.max(10, parameters.diffusion)),
        );
        samples[index] =
          filtered *
          attack *
          Math.exp(-6.9 * time / duration) *
          0.18 *
          lateGain;
      }
      ...
      reflections.forEach((time, reflection) => {
        const start = Math.round(
          (time + channel * (reflection + 1) * 0.0007) *
            size *
            sampleRate,
        );
        ...
        for (
          let index = 0;
          index < width && start + index < samples.length;
          index += 1
        ) {
          const polarity = reflection % 2 === 0 ? 1 : -1;
          const reflectionGain = 0.45 ** (reflection / 2);
          const decay = Math.exp(-index / (sampleRate * 0.00025));

          samples[start + index] +=
            polarity * reflectionGain * decay * earlyGain;
        }
      });
    },
    parameters.lowCutHz,
      ...
      roundTrips.forEach((roundTrip, spring) => {
        ...
        for (let echo = 0; ; echo += 1) {
          ...
          const width = Math.round(
            (0.055 + spring * 0.008) * sampleRate,
          );
          const amplitude =
            Math.exp(-6.9 * onset / duration) / (1 + spring * 0.3);
          ...
          for (
            ...
            const dispersion =
              0.006 + spring * 0.002 + echo * 0.0001;
            const sweep =
              (bright ? 4_200 : 2_100) / (1 + echo * 0.04);
            const phase =
              2 * Math.PI *
              ((bright ? 520 : 340) * time +
                sweep *
                  dispersion *
                  (1 - Math.exp(-time / dispersion)));
            const envelope =
              Math.sin(Math.PI * index / width) ** 2 *
              Math.exp(-time * 65);
...
function createDigitalImpulse(
  ...
  const duration =
    decay * (hall ? 3.4 / 2.8 : 1.1 / 0.85) +
    Math.max(
      0,
      parameters.preDelayMs / 1000 - (hall ? 0.028 : 0.008),
    );
    ...
    (samples, channel, sampleRate) => {
      ...
      for (const seconds of [0.005, 0.0017, 0.0006]) {
        const delay = Math.max(
          1,
          Math.round(
            (seconds * size + channel * 0.00013) * sampleRate,
          ),
        );
        const line = new Float32Array(delay);
