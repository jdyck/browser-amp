...
- [ ] Compression and Reverb Stage Bypass preserve Amount; Reverb bypass has the expected tail chop without a pop.
...
- [ ] Clean Gain, Bass, Middle, Treble, Compression Amount, Reverb Amount, and Master Volume all produce the expected audible change.
- [ ] Exact numeric entry, clamping, slider synchronization, arrow keys, and focus visibility work for every continuous control.
- [ ] Compression Stage Bypass preserves Amount and switches without an obvious click.
...
- [ ] Release notes still name Chrome on macOS as supported and keep iPhone browsers and Noise Gate outside first-release scope.
