...
- Settings are one flat `AmpControlSettings` object, and persistence already does useful runtime normalization plus a top-level version check ([control normalization](../../src/signalChain/settings.ts), [preferences store](../../src/preferences.ts)).
